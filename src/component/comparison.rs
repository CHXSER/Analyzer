...
use crate::{model::media::DuplicateMedia, DUPS};
...
// TODO: Aggiungere encoding della path per risolvere problemi
#[component]
fn MediaDisplayRight(media: DuplicateMedia) -> Element {
    rsx! {
        div { class: "media-container",
            match media {
                DuplicateMedia::PhotoMatchGroup(a) => {
                    println!("{:?}", a.images[1].path);
                    rsx! {
                        img { class: "media", src: a.images[1].path.clone().to_str().unwrap_or("") }
                    }
                }
                DuplicateMedia::VideoMatchGroup(a) => {
